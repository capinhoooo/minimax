...
export class LiFiIntegration {
     ...
     */
    async getSwapQuote(fromChainId, toChainId, fromToken, toToken, fromAmount, userAddress) {
        try {
            const quoteRequest = {
     ...
     */
    async getSwapRoutes(fromChainId, toChainId, fromToken, toToken, fromAmount, userAddress) {
        try {
            const routesRequest = {
                fromChainId,
                toChainId,
     ...
     */
    calculateLPSplit(totalAmountUSD, token0PriceUSD, token1PriceUSD, currentTick, tickLower, tickUpper) {
    ...
    async getLPEntryQuotes(params) {
        ...
        const [quote0, quote1] = await Promise.all([
            ...
            this.getSwapQuote(params.fromChainId, params.toChainId, params.fromToken, params.toToken0, halfAmount, params.userAddress),
            // Quote for second token of LP pair
            this.getSwapQuote(params.fromChainId, params.toChainId, params.fromToken, params.toToken1, halfAmount, params.userAddress),
        ]);
     ...
     */
    async prepareSwapExecution(route) {
        logger.logAction({
     ...
     */
    async checkTransactionStatus(txHash, fromChainId, toChainId) {
        try {
