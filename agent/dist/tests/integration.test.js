...
import { ArcIntegration } from '../integrations/ArcIntegration.js';
// Test wallet address
...
async function testCrossChainEntryFlow() {
    ...
    try {
        ...
        const directRoutes = await lifi.getSwapRoutes(SUPPORTED_CHAINS.ARBITRUM, SUPPORTED_CHAINS.BASE, flow.inputToken, flow.battlePool.token1, // USDC on Base
        flow.inputAmount.toString(), TEST_ADDRESS);
    ...
    const { amount0, amount1 } = lifi.calculateLPSplit(BigInt(1000e8), // 1000 USD total
    ethPriceUSD, usdcPriceUSD, currentTick, flow.battlePool.tickLower, flow.battlePool.tickUpper);
    ...
    try {
        ...
        const wethQuote = await lifi.getSwapQuote(SUPPORTED_CHAINS.BASE, SUPPORTED_CHAINS.BASE, flow.battlePool.token1, // USDC
        flow.battlePool.token0, // WETH
        halfUSDC, TEST_ADDRESS);
        if (wethQuote) {
