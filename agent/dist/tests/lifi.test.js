...
async function testLiFiIntegration() {
    ...
    try {
        const quote = await lifi.getSwapQuote(SUPPORTED_CHAINS.ARBITRUM, SUPPORTED_CHAINS.ARBITRUM, TOKENS.USDC[SUPPORTED_CHAINS.ARBITRUM], TOKENS.WETH[SUPPORTED_CHAINS.ARBITRUM], '1000000000', // 1000 USDC (6 decimals)
        TEST_ADDRESS);
    ...
    try {
        const quote = await lifi.getSwapQuote(SUPPORTED_CHAINS.ARBITRUM, SUPPORTED_CHAINS.BASE, TOKENS.USDC[SUPPORTED_CHAINS.ARBITRUM], TOKENS.WETH[SUPPORTED_CHAINS.BASE], '500000000', // 500 USDC
        TEST_ADDRESS);
        if (quote && BigInt(quote.toAmount) > 0n) {
    ...
    try {
        const routes = await lifi.getSwapRoutes(SUPPORTED_CHAINS.ARBITRUM, SUPPORTED_CHAINS.BASE, TOKENS.USDC[SUPPORTED_CHAINS.ARBITRUM], TOKENS.USDC[SUPPORTED_CHAINS.BASE], '100000000', // 100 USDC
        TEST_ADDRESS);
        if (routes.length > 0) {
    ...
    try {
        const split = lifi.calculateLPSplit(BigInt(2000e8), // 2000 USD (8 decimals for consistency)
