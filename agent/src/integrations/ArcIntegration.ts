...
export class ArcIntegration {
  private clients: Map<SupportedChain, ReturnType<typeof createPublicClient>> = new Map();
  private attestationServiceUrl = 'https://iris-api.circle.com/attestations';
  ...
  private initializeClients() {
    ...
    for (const [name, config] of Object.entries(chains)) {
      this.clients.set(name as SupportedChain, createPublicClient({
        chain: config.chain,
   ...
   */
  async getUSDCBalance(chain: SupportedChain, address: Address): Promise<bigint> {
  ...
  prepareDepositForBurnData(request: BridgeRequest): {
    ...
    const mintRecipient = `0x000000000000000000000000${request.recipient.slice(2)}` as `0x${string}`;
   ...
   */
  async getAttestation(messageHash: string): Promise<{ attestation: string; status: string } | null> {
    try {
  ...
  getBridgeInstructions(request: BridgeRequest): {
    steps: Array<{
      ...
      action: string;
      description: string;
      transaction?: { to: Address; data: `0x${string}` };
    }>;
  } {
   ...
   * Log a bridge action
   */
  logBridgeAction(action: string, request: BridgeRequest, status: string, txHash?: string) {
    logger.logAction({
