...
async function testArcIntegration() {
  ...
  try {
    const chains: SupportedChain[] = ['ETHEREUM', 'ARBITRUM', 'BASE', 'POLYGON', 'OPTIMISM'];
    let allValid = true;
  ...
  try {
    const request = {
      sourceChain: 'ARBITRUM' as SupportedChain,
      destChain: 'BASE' as SupportedChain,
  ...
  try {
    const request = {
      sourceChain: 'POLYGON' as SupportedChain,
      destChain: 'BASE' as SupportedChain,
  ...
  try {
    ...
    for (const [chain, expectedDomain] of Object.entries(expectedDomains)) {
      const contracts = CCTP_CONTRACTS[chain as SupportedChain];
  ...
  try {
    ...
    const hasAll = requiredChains.every(c => chains.includes(c as SupportedChain));
